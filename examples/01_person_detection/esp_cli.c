...
// 使用 asm 關鍵字可以將組合語言指令嵌入到程式碼中的特定位置。
// 這樣可以在高階語言的程式碼中直接使用組合語言的功能，例如使用特定的處理器指令或訪問特定的硬體資源。
// 嵌入二進位資料的好處包括
// 1. 簡化部署：不需要單獨管理外部資源文件，只需一個編譯後的應用程式文件。
// 2. 提高執行效率：嵌入的資料可以直接在記憶體中存取，而不需要文件系統的讀取操作，因此可以提高資料的讀取速度。
// 3. 避免外部資源的依賴問題：嵌入資料可確保程式運行時所需的資源始終可用，而不受外部文件是否存在或可讀的影響。
...
// 打印當前系統中所有任務的信息，包括任務名稱、任務號、優先級和堆棧使用情況。
...
// 打印内存使用情况，包括当前可用内存、最大可用内存和最小可用内存。
...
// 进行图像推理，根据传入的图像编号选择对应的图像进行推理，并计算推理所需的时间。
