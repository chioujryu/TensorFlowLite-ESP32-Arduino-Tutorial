...
#include "tensorflow/lite/micro/micro_mutable_op_resolver.h" // 測試用
...
void loop() {
  ...
  int8_t k_x_1_quantized = k_x_1 / input->params.scale + input->params.zero_point;
  input->data.int8[0] = k_x_1_quantized;

  int8_t k_x_2_quantized = k_x_2 / input->params.scale + input->params.zero_point;
  ...
  if (invoke_status != kTfLiteOk) {
    TF_LITE_REPORT_ERROR(error_reporter, "Invoke failed on %f, %f",
                         static_cast<double>(k_x_1),static_cast<double>(k_x_2) );
    return;
  }
